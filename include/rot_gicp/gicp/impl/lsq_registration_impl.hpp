...
void LsqRegistration<PointTarget, PointSource>::computeTranslation(PointCloudSource& output, Eigen::Vector3d& trans,
  ...
  for (int i = 0; i < max_iterations_ && !t_converged_; i++) {
    nr_iterations_ = i;
    Eigen::Vector3d delta_t;  // Iteration increment
...
bool LsqRegistration<PointTarget, PointSource>::step_t_optimize(Eigen::Vector3d& x0, Eigen::Vector3d& delta,
  ...
  Eigen::Matrix<double, 6, 6> H;  // Hessian matrix
  Eigen::Matrix<double, 6, 1> b;  // Gradient vector
  
  // std::cout << "x0: " << x0.matrix() << std::endl;
  double y0 = t3_linearize(x0, init_guess, last_t0, interval_tn, interval_tn_1, &H, &b); // Initial error and transform
  ...
  for (int i = 0; i < lm_max_iterations_; i++) {
    ...
    Eigen::Isometry3d delt_t = se3_exp(d); // SE3 update
    delta = delt_t.translation();

    // delta = d;
    // std::cout << "t_delta: " << delta.transpose() << std::endl;
    Eigen::Vector3d xi = delta + x0;
    double yi = compute_t_error(xi, init_guess, last_t0, interval_tn, interval_tn_1);  // Compute updated error
    double rho = (y0 - yi) / (d.dot(lm_lambda_ * d - b)); // LM gain ratio
    ...
    if (rho < 0) {
      ...
      nu = 2 * nu;  // Increase LM step
      continue;
    }
    // Reduce damping
    x0 = xi;
    lm_lambda_ = lm_lambda_ * std::max(1.0 / 3.0, 1 - std::pow(2 * rho - 1, 3));
    // so3_final_hessian_ = H; // Store latest Hessian
    return true;
  }
...
void LsqRegistration<PointTarget, PointSource>::computeTransformation(PointCloudSource& output, const Matrix4& guess) {
  ...
  for (int i = 0; i < max_iterations_ && !converged_; i++) {
    nr_iterations_ = i;
    // Euclidean transform: [R | t]
    Eigen::Isometry3d delta;  // Iteration increment
...
bool LsqRegistration<PointTarget, PointSource>::step_lm(Eigen::Isometry3d& x0, Eigen::Isometry3d& delta) {
  Eigen::Matrix<double, 6, 6> H;  // Hessian matrix
  Eigen::Matrix<double, 6, 1> b;  // Gradient vector
  double y0 = linearize(x0, &H, &b); // Initial error and transform
  if (lm_lambda_ < 0.0) {
    lm_lambda_ = lm_init_lambda_factor_ * H.diagonal().array().abs().maxCoeff();
  }
  ...
  for (int i = 0; i < lm_max_iterations_; i++) {
    ...
    delta = se3_exp(d); // SE3 update

    Eigen::Isometry3d xi = delta * x0;
    double yi = compute_error(xi);  // Compute updated error
    double rho = (y0 - yi) / (d.dot(lm_lambda_ * d - b)); // LM gain ratio
    ...
    x0 = xi;
    lm_lambda_ = lm_lambda_ * std::max(1.0 / 3.0, 1 - std::pow(2 * rho - 1, 3));
    final_hessian_ = H; // Hessian matrix
    return true;
  }
...
bool LsqRegistration<PointTarget, PointSource>::rot_step_lm(Eigen::Isometry3d& x0, Eigen::Isometry3d& delta) {
  ...
  Eigen::Matrix<double, 3, 1> b;  // Gradient vector
  
  // std::cout << "x0: " << x0.matrix() << std::endl;
  double y0 = so3_linearize(x0, &H, &b); // Initial error and transform
  // std::cout << "H: " << std::endl << H << std::endl;
  ...
  for (int i = 0; i < lm_max_iterations_; i++) {
    ...
    // Eigen::Vector3d d_v(d(0), d(1), d(2));
    Eigen::Quaterniond q_delta = so3_exp(d); // SO3 update
    delta.setIdentity();
    delta.matrix().block<3,3>(0,0) = q_delta.toRotationMatrix();  // Assign rotation block
    ...
    x0 = xi;
    lm_lambda_ = lm_lambda_ * std::max(1.0 / 3.0, 1 - std::pow(2 * rho - 1, 3));
    so3_final_hessian_ = H; // Hessian matrix
    return true;
  }
