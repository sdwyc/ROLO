...
public:
    ...
    ParamLoader()
    {
        ...
        nh.param<std::string>("rolo/slopeTopic", slopeTopic, "my_slope");
        ...
        nh.param<bool>("rolo/useCloudRing", useCloudRing, false);
        ...
        nh.param<int>("rolo/groundScanInd",groundScanInd, 7);
        nh.param<float>("rolo/scanPeriod", scanPeriod, 0.1);
        nh.param<int>("rolo/systemDelay", systemDelay, 0);
        // nh.param<int>("slopeLO/imuQueLength", imuQueLength, 200);
        nh.param<float>("rolo/sensorMinimumRange", sensorMinimumRange, 1.0);
        nh.param<float>("rolo/sensorMountAngle", sensorMountAngle, 0.0);
        nh.param<float>("rolo/segmentTheta", segmentTheta, 60.0/180.0*M_PI);
        nh.param<int>("rolo/segmentValidPointNum", segmentValidPointNum, 5);
        nh.param<int>("rolo/segmentValidLineNum", segmentValidLineNum, 3);
        segmentAlphaX = ang_res_h/180.0 * M_PI;
        segmentAlphaY = ang_res_v / 180.0 * M_PI;
        nh.param<int>("rolo/edgeFeatureNum", edgeFeatureNum, 4);
        nh.param<int>("rolo/surfFeatureNum", surfFeatureNum, 8);
        nh.param<int>("rolo/surfFeatureNum", sectionsTotal, 12);

        nh.param<float>("rolo/edgeThreshold", edgeThreshold, 0.1);
        nh.param<float>("rolo/surfThreshold", surfThreshold, 0.1);
        nh.param<float>("rolo/nearestFeatureSearchSqDist", nearestFeatureSearchSqDist, 25);
        ...
        nh.param<bool>("rolo/loopClosureEnableFlag", loopClosureEnableFlag, false);
        ...
        nh.param<float>("rolo/historyKeyframeSearchRadius", historyKeyframeSearchRadius, 10.0);//默认值不同
        ...
        nh.param<float>("rolo/globalMapVisualizationSearchRadius", globalMapVisualizationSearchRadius, 1e3);//默认值不同
        ...
        nh.param<int>("rolo/surroundingKeyframeSearchNum", surroundingKeyframeSearchNum, 50);
        // slopeEstimation
        nh.param<std::string>("rolo/globalframe",robotFrame,"base_link");
        nh.param<float>("rolo/anglebias", anglebias, 0.5);
        nh.param<float>("rolo/detectionXMin",detectionXMin, 3.0);
        nh.param<float>("rolo/detectionXMax",detectionXMax, 15.0);
        nh.param<float>("rolo/detectionYMin",detectionYMin, -1.0);
        nh.param<float>("rolo/detectionYMax",detectionYMax, 1.0);
        nh.param<float>("rolo/detectionZMin",detectionZMin, -2);
        nh.param<float>("rolo/detectionZMax",detectionZMax, 2);

        nh.param<bool>("rolo/saveTraj",saveTraj,false);
        nh.param<std::string>("rolo/filepath",filepath,"/home/chen/IROS2022/kittislope.txt");
        nh.param<std::string>("rolo/fileDirectory",fileDirectory,"/tmp/");

        nh.param<int>("rolo/intervalTime",interTime, 7);
        nh.param<float>("rolo/ThresholdVTDis",slopeDisThre, 0.7);
        nh.param<float>("rolo/sensorHeight",sensorHeight,1.9);
        nh.param<float>("rolo/plainThre", plainThre, 1.5);
        nh.param<float>("rolo/minAngle",minAngle,-4.0);
        nh.param<float>("rolo/maxAngle",maxAngle,4.0);
