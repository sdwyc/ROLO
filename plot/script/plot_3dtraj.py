...
def plot_trajectory(tum_file, dir_name=None, background_color='white', 
    ...
    ax.w_xaxis.set_pane_color((1.0, 1.0, 1.0, 1.0))  # RGB white, with alpha 1
    ...
    if os.path.basename(tum_file[0]) == 'GroundTruth.tum':
        ...
        ax.plot(read_tum(tum_file[0])[0]['x'], read_tum(tum_file[0])[0]['y'], read_tum(tum_file[0])[0]['z'], 
                color = '#808A87', linestyle='--',label=read_tum(tum_file[0])[1])
    ...
    x_interval, y_interval, z_interval = 80, 80, 80
    if dir_name=='KITTI00':
        x_min, x_max = -100, 500
        y_min, y_max = -300, 300
        z_min, z_max = -300, 300
