...
def plot_trajectory(tum_file, dir_name=None, background_color='white', 
    ...
    if os.path.basename(tum_file[0]) == 'GroundTruth.tum':
        ...
        ax.plot(read_tum(tum_file[0])[0]['timestamp']-start_time,read_tum(tum_file[0])[0]['z'], color = '#808A87', 
                linestyle='--',linewidth=2.75,label=read_tum(tum_file[0])[1])
        ...
        for i in range(0,tum_lenth):
            ...
            ax.plot(read_tum(tum_file[i])[0]['timestamp']-start_time,read_tum(tum_file[i])[0]['z'], color = trajectory_color[i], 
                    linestyle=trajectory_linestyle,linewidth=2,label=read_tum(tum_file[i])[1])
    ...
    if dir_name=='qianfo':
        ax.set_xlim(-5,525)  # 设置 x 轴范围为 0 到 6
        ax.set_ylim(-140, 160)  # 设置 y 轴范围为 0 到 30
    if dir_name=='xinglong':
        ax.set_xlim(-5, 530)  # 设置 x 轴范围为 0 到 6
        ax.set_ylim(-222, 225)  # 设置 y 轴范围为 0 到 30
    if dir_name=='Offroad1':
        ax.set_xlim(-5, 190)  # 设置 x 轴范围为 0 到 6
        ax.set_ylim(-350, 250)  # 设置 y 轴范围为 0 到 30
    if dir_name=='Offroad2':
        ax.set_xlim(-5, 199)  # 设置 x 轴范围为 0 到 6
        ax.set_ylim(-120, 70)  # 设置 y 轴范围为 0 到 30
    if dir_name=='Offroad3':
        ax.set_xlim(-15, 450)  # 设置 x 轴范围为 0 到 6
        ax.set_ylim(-160, 220)  # 设置 y 轴范围为 0 到 30
...
def plot(map):
    ...
    trajectories_color = {
        # 'ROLO': (0, 1.0, 0.0),  # Blue
        # 'ROLO+LC': (0, 1.0, 0.0),  # Blue
        'ROLO': (1.0, 0., 0.),   # Purple
        'ROLO+LC': (1.0, 0., 0.),   # Purple
        'LOAM': (0., 0., 1),   # Purple
