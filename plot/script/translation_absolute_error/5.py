...
def plot_trajectory(tum_file,tum_name1, background_color='white', 
    ...
    ax.plot(1*(read_tum(tum_name1)['timestamp']-start_time),read_tum(tum_name1)['x'], color = trajectory_color[0], 
            linestyle=trajectory_linestyle,label='x',linewidth=3)
    ax.plot(1*(read_tum(tum_file)['timestamp']-start_time),read_tum(tum_file)['y'], color = trajectory_color[1], 
            linestyle=trajectory_linestyle,label='y',linewidth=3)
    ax.plot(1*(read_tum(tum_file)['timestamp']-start_time),read_tum(tum_file)['z'], color = trajectory_color[2], 
            linestyle=trajectory_linestyle,label='z',linewidth=3)
    ...
    ax.yaxis.set_major_locator(MultipleLocator(2))  # y轴每隔5一个刻度
# 定义鼠标移动事件处理程序
    def on_move(event):
        if event.inaxes == ax:
            x = event.xdata
...
tum_name = '/home/abin/tum_traj/ERROR/translation/kitti05/plot.tum'
tum_name1 = '/home/abin/tum_traj/ERROR/translation/kitti05/plot.tum'

trajectories_color = ['#00FF00','#0000FF','#9933FA']
