...
class DualOdomDeltaStateNode(object):
    def __init__(self):
        ...
        self.future_local_line_marker_topic = rospy.get_param(
            "~future_local_line_marker_topic",
            self.future_local_marker_topic + "_line",
        )
        ...
        self.pub_marker = rospy.Publisher(self.marker_topic, PoseWithCovarianceStamped, queue_size=10)
        ...
        self.pub_future_global_marker = rospy.Publisher(self.future_global_marker_topic, PoseWithCovarianceStamped, queue_size=10)
        self.pub_future_local_marker = rospy.Publisher(self.future_local_marker_topic, PoseWithCovarianceStamped, queue_size=10)
        self.pub_future_local_line_marker = rospy.Publisher(self.future_local_line_marker_topic, Marker, queue_size=10)
    ...
    def build_pose_msgs(self, stamp, px, py, pz, qx, qy, qz, qw, pose_cov, frame_id=None):
        pose_msg = PoseWithCovarianceStamped()
        pose_msg.header.stamp = stamp
        pose_msg.header.frame_id = self.world_frame if frame_id is None else frame_id
        ...
        pose_msg.pose.covariance = np.asarray(pose_cov, dtype=float).reshape(-1).tolist()
        return pose_msg
    ...
    def publish_timer_cb(self, _event):
        with self.lock:
            ...
            current_marker = self.build_pose_msgs(
                stamp=current_stamp,
                px=cpx,
                py=cpy,
                pz=cpz,
            ...
            if future_dt > 1e-9:
                ...
                if self.publish_future_global_odom and self.pub_future_global_odom is not None:
                    ...
                    future_global_marker = self.build_pose_msgs(
                        stamp=future_stamp,
                        px=future_pos[0],
                        py=future_pos[1],
                        pz=future_pos[2],
                ...
                if self.publish_future_local_odom and self.pub_future_local_odom is not None:
                    ...
                    future_local_axes_marker = self.build_pose_msgs(
                        stamp=future_stamp,
                        px=lpx,
                        py=lpy,
                        pz=lpz,
                    ...
                    self.pub_future_local_line_marker.publish(future_local_line_marker)
